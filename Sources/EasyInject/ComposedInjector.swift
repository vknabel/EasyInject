...
public struct ComposedInjector<K: ProvidableKey>: InjectorDerivingFromMutableInjector {
  ...
  public mutating func resolve(key: Key) throws -> Providable {
    do {
      ...
      do {
        ...
        throw ComposedInjector<Key>.rethrowResolvingErrors(
          forKey: key, left: leftError, right: rightError)
      }
    }
  }
  ...
  private static func rethrowResolvingErrors(
    forKey key: Key, left leftError: Error, right rightError: Error
  ) -> Error {
  ...
  public mutating func provide(
    key: Key, usingFactory factory: @escaping (inout ComposedInjector) throws -> Providable
  ) {
...
extension ComposedInjector {
  ...
  public mutating func resolveLeft<Value: Providable>(
    from provider: Provider<Key, Value>
  ) throws -> Value {
  ...
  public mutating func provideLeft(
    key: Key, usingFactory factory: @escaping (inout ComposedInjector) throws -> Providable
  ) {
    var this = self
    left.provide(
      key: key,
      usingFactory: { newMutable in
  ...
  public mutating func provideRight(
    key: Key, usingFactory factory: @escaping (inout ComposedInjector) throws -> Providable
  ) {
    var this = self
    right = right.providing(
      key: key,
      usingFactory: { newMutable in
  ...
  public mutating func provideBoth(
    key: Key, usingFactory factory: @escaping (inout ComposedInjector) throws -> Providable
  ) {
    provideLeft(key: key, usingFactory: factory)
  ...
  public mutating func provideLeft<Value: Providable>(
    ...
    left = left.providing(
      for: provider,
      usingFactory: { newMutable -> Value in
        this.left = newMutable
        return try factory(&this)
      })
  }
...
extension ComposedInjector {
  ...
  /**
     ...
     - Throws: `InjectionError`, may throw `.customError(ComposedInjectionError)`
     */
  public func resolvingBoth<Value: Providable>(from provider: Provider<Key, Value>) throws -> (
    Value, Value
  ) {
