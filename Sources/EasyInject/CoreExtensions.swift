extension Injector {
  #if swift(>=4.0)
    public subscript<Value: Providable>(provider: Provider<Key, Value>) -> Value? {
  ...
  public func revoking<Value: Providable>(for provider: Provider<Key, Value>) -> Self {
...
extension InjectorDerivingFromMutableInjector {
  ...
  public func providing(
    key: Self.Key, usingFactory factory: @escaping (inout Self) throws -> Providable
  ) -> Self {
    var copy = self.copy()
...
extension MutableInjector where Self: AnyObject {
  /**
     Additionally provides a value for a given `Provider`.
